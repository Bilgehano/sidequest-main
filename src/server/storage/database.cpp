...
namespace Sidequest
{
	...
	namespace Server
	{
		...
		Database::Database(std::string filepath_of_database)
		{
			...
			if (!(is_open)) {
				std::cout << "Data intialize  " << std::endl;
				initialize_schema();
			}
		...
		void Database::initialize_schema() {
			const std::string schema_sql = R"(
				...
				INSERT OR IGNORE INTO user(email, display_name, password)
				VALUES ('sidequest_root@hs-aalen.de', 'Sidequest Root User', '');
			)";

			int result = sqlite3_exec(handle, schema_sql.c_str(), nullptr, nullptr, nullptr);
			if (result != SQLITE_OK) {
				throw std::runtime_error("Failed to execute schema SQL");
			}
		}
